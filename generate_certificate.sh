...
caBundle=$(oc extract -n openshift-kube-controller-manager secret/csr-signer --to - --keys  tls.crt 2>/dev/null | base64 -w 0)
...
while true; do
  ...
  if oc patch "${kind:-mutatingwebhookconfiguration}" "${webhook}" --type='json' -p "[{'op': 'add', 'path': '/webhooks/0/clientConfig/caBundle', 'value':'${caBundle}'}]"; then
      break
  fi
